...
def main():
    ...
    theory_cards = int(data.get("theory_cards", 0))
    ...
    if args.budget:
        ...
        target_total_min = int(budget.get("target_total_cards_min", 0))
        target_total_max = int(budget.get("target_total_cards_max", max_cards_per_note))
        max_cards_per_note = target_total_max or max_cards_per_note

        target_constructive_min = int(budget.get("target_constructive_min", 0))
        target_constructive_max = int(
            budget.get("target_constructive_max", total_cards)
        )
        target_theory_max = int(budget.get("target_theory_max", total_cards))
        constructive_min_ok = (
            True
            if total_cards < target_constructive_min
            else constructive_cards >= target_constructive_min
        )
        constructive_max_ok = constructive_cards <= target_constructive_max

        budget_ok = (
            total_cards >= target_total_min
            and total_cards <= target_total_max
            ...
            and constructive_min_ok
            and constructive_max_ok
            and theory_cards <= target_theory_max
        )
        if not budget_ok:
            failures.append("budget_mismatch")
        if not constructive_max_ok:
            failures.append("constructive_cap_exceeded")
    ...
    conversion_quota_ok = True
    if procedure_cards >= 5:
        required_conversions = procedure_cards // 5
        deep_cards = (
            model_cards
            + failure_mode_cards
        ...
        if deep_cards < required_conversions:
            conversion_quota_ok = False
            failures.append("conversion_quota")

    deep_ratio_ok = True
    constructive_ratio_ok = True
    ...
    all_checks_passed = (
        ...
        and budget_ok
        and conversion_quota_ok
        and deep_ratio_ok
        and constructive_ratio_ok
        and quality_ok
    ...
    report = {
        "doctrine_compliance_report": {
            ...
            "budget_ok": budget_ok,
            "conversion_quota_ok": conversion_quota_ok,
            "deep_ratio_ok": deep_ratio_ok,
            "constructive_ratio_ok": constructive_ratio_ok,
