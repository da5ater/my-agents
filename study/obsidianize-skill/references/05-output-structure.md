...
2.  **Machine-Parsable Optimization (Extraction-Ready):** Structure the content so a downstream system can instantly identify facts for structured extraction. Use **Declarative Statements** (Bold key concept -> explain detail).
...
#### Output Rules (File Only)

- **emit=file only:** Write the final note as a `.md` file in the **current working directory**.
- **Stop if unavailable:** If file-writing capability is absent, STOP and report the limitation.
...
- **Single Write Operation:** Write the entire note in ONE operation. Do NOT create a partial file and then edit it.
- **No artifacts in file:** The file must contain only the final Obsidian note content.

#### Validator Script Gate (Required)

- Validate the note string before writing, using a temp file or stdin if needed: `python scripts/validate_obsidian_note.py <note_path>` or `python scripts/validate_obsidian_note.py -`
- If validation fails: follow Failure Protocol (targeted regeneration only, max 3 attempts), then re-validate before writing.
- Emit the validator output (pass/fail + errors) in chat.
