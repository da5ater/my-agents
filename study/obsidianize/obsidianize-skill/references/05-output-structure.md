...
#### Output Rules (Capability-Gated)

- **emit=chat:** Output the full note as Markdown in chat.
- **emit=file:** If file-writing capability exists, write the final note as a `.md` file in the **current working directory**.
- **Fallback:** If emit=file is requested but file-writing capability is absent, fallback to emit=chat.
- **Filename Rule (emit=file only):** Use the note title as filename. Sanitize unsafe characters.
- **Fallback Filename (emit=file only):** Use **current directory name** if title unclear.
- **No H1 in Body:** Do NOT include the title as an H1 inside the note. Title lives only in filename.
- **Single Write Operation (emit=file only):** Write the entire note in ONE operation. Do NOT create a partial file and then edit it.
